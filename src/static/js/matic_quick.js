...
async function loadDQuick(App, prices){
  const dQUICK_ADDR = "";
  const QUICK_ADDR = "0x831753DD7087CaC61aB5644b308642cc1c33Dc13";
  const dQUICK_CONTRACT = new ethers.Contract(dQUICK_ADDR, xPEFI_ABI, App.provider);
  const QUICK_CONTRACT = new ethers.Contract(QUICK_ADDR, QUICK_ABI, App.provider);
  const totalDepositedQuick = await QUICK_CONTRACT.balanceOf(dQUICK_ADDR) / 1e18;
  const totalDQuickTokens = await QUICK_CONTRACT.totalSupply() / 1e18;
  const virtualPrice = totalDepositedQuick / totalDQuickTokens;
  const quickPrice = getParameterCaseInsensitive(prices, QUICK_ADDR).usd;
  const dQuickPrice = quickPrice * virtualPrice;
  const rewardTicker = await dQUICK_CONTRACT.symbol();
  const stakeTicker = await QUICK_CONTRACT.symbol();
  const totalOwnedDQuick = await dQUICK_CONTRACT.balanceOf(App.YOUR_ADDRESS) / 1e18;
  const totalOwnedQuick = totalOwnedDQuick * virtualPrice;
  const tvl = totalDepositedQuick * quickPrice
  const usersPercentage = totalOwnedDQuick / totalDQuickTokens * 100;
  const usersDQuickUsd = totalOwnedDQuick * quickPrice;
  _print(`\n${rewardTicker} Price: $${toFixed(dQuickPrice, 2)} Total Supply: ${toFixed(totalDQuickTokens, 4)} TVL: $${formatMoney(tvl)}`);
  _print(`${stakeTicker} Price: $${toFixed(quickPrice, 2)}`);
  _print(`There is a total of ${toFixed(totalDepositedQuick, 2)} QUICK staked in dQUICK`);
  _print(`Your balance is ${totalOwnedDQuick.toFixed(2)} ${rewardTicker} (${totalOwnedQuick.toFixed(2)} ${stakeTicker}), $${formatMoney(usersDQuickUsd)}, ${usersPercentage.toFixed(2)}% of the pool`);
  const approveAndStake = async function() {
    return contract_stake(xPEFI_ABI, dQUICK_ADDR, App, QUICK_ADDR)
  }      
  const unstake = async function() {
    return contract_unstake(xPEFI_ABI, dQUICK_ADDR, totalOwnedDQuick, App)
  }
  _print_link(`Stake ${totalOwnedQuick.toFixed(2)} ${stakeTicker}`, approveAndStake)
  _print_link(`*** Paper hands penalty ***`);
  _print_link(`Unstaking currently incurs a 25% fee.`);
  _print_link(`Unstake ${totalOwnedDQuick.toFixed(2)} ${rewardTicker}`, unstake)
}
